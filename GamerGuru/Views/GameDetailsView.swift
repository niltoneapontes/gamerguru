    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
            ...
            VStack {
                ...
                Text(gameListViewModel.getSelectedGame().name ?? "Sem nome").frame(maxWidth: /*@START_MENU_TOKEN@*/.infinity/*@END_MENU_TOKEN@*/).font(.title).foregroundColor(.white).padding(.vertical, 24)
                Text(gameListViewModel.getSelectedGame().summary ?? "Sem resumo").frame(maxWidth: /*@START_MENU_TOKEN@*/.infinity/*@END_MENU_TOKEN@*/).font(.title2).foregroundColor(.white).padding(.bottom, 32)
                Spacer()

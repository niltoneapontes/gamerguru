...
class GameListViewModel: ObservableObject {
    ...
    func getGameListByGenre(genre: Genre) {
        let url = URL(string: "https://api.igdb.com/v4/games")!
        var requestHeader = URLRequest.init(url: url)
        requestHeader.httpMethod = "POST"
        requestHeader.setValue("cvg9d0poxp7c42q8bxfbakkjz3okm7", forHTTPHeaderField: "Client-ID")
        ...
        requestHeader.httpBody = "fields: cover,genres,language_supports,name,platforms,rating,rating_count,screenshots,summary,url; where genres=(\(genre.id));".data(using: .utf8)
        ...
        URLSession.shared.dataTask(with: requestHeader) { [weak self] data, response, error in
            ...
            if let data = data {
                do {
                    ...
                    print("Error converting data to JSON at Games by Genre: \(error.localizedDescription)")
                }
            }
        }.resume()
    }
